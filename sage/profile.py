...
Everything deployment-specific used to be data pretending to be code: a base URL
in `config`, a slug rule in `corpus`, a synonym table in `search`, tool copy in
`tools`, a system prompt of its own, starter cards and a page title in `app.py`,
...
instances; nothing else in the package should name a particular deployment.
...
class Profile:
    ...
    system_prompt: str = ""
    # The corpus in a few words, as it appears in tool descriptions — for this
    # deployment, "Y. Yu's blog articles".
    ...
    # The composer's placeholder. It was the last piece of copy still hardcoded in

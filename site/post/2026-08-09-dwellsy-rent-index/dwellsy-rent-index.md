...
[figure: Figure 1. Standard deviation of the repeat-sales residual by the gap between the two listings of a pair. The generalised least squares stage weights each pair by the inverse of this fitted variance. The dip at nine to twelve months is the annual lease cycle: pairs almost exactly a year apart are unusually well behaved.] Figure 1: Figure 1. Standard deviation of the repeat-sales residual by the gap between the two listings of a pair. The generalised least squares stage weights each pair by the inverse of this fitted variance. The dip at nine to twelve months is the annual lease cycle: pairs almost exactly a year apart are unusually well behaved.

The second requirement is that a repeat-sales index is only identified in months that actually contain pairs.

[figure: Figure 2. A pair is dated by its second listing, so the count is mechanically zero at the start of the window and thin for the first half-year; the index is reported from 2021-01 for that reason. The upward drift thereafter is the panel accumulating history, not a rising re-listing rate.] Figure 2: Figure 2. A pair is dated by its second listing, so the count is mechanically zero at the start of the window and thin for the first half-year; the index is reported from 2021-01 for that reason. The upward drift thereafter is the panel accumulating history, not a rising re-listing rate.
...
[figure: Figure 3. All three series are indexed to 100 in January 2021. The Dwellsy index reaches its peak and turns over well before either comparison series does; the CPI, which is contract rent paid by all tenants rather than the rent asked of new ones, is still climbing when asking rents have been flat for a year.] Figure 3: Figure 3. All three series are indexed to 100 in January 2021. The Dwellsy index reaches its peak and turns over well before either comparison series does; the CPI, which is contract rent paid by all tenants rather than the rent asked of new ones, is still climbing when asking rents have been flat for a year.
...
[figure: Figure 5. Each point is a metropolitan area; the diagonal is equality. The rank correlation is 0.92, so the index orders metros almost exactly as ZORI does — but every one of the 46 points lies below the diagonal, which is a systematic level difference and not scatter.] Figure 5: Figure 5. Each point is a metropolitan area; the diagonal is equality. The rank correlation is 0.92, so the index orders metros almost exactly as ZORI does — but every one of the 46 points lies below the diagonal, which is a systematic level difference and not scatter.
...
[figure: Figure 8. Six ways of measuring the same quantity on the same data, against the two external series. The Dwellsy estimators differ in almost every respect — one holds the unit fixed, one holds the composition fixed, one adjusts for observable quality — and they agree with each other far more closely than any of them agrees with ZORI or the CPI.] Figure 8: Figure 8. Six ways of measuring the same quantity on the same data, against the two external series. The Dwellsy estimators differ in almost every respect — one holds the unit fixed, one holds the composition fixed, one adjusts for observable quality — and they agree with each other far more closely than any of them agrees with ZORI or the CPI.
...
[figure: Figure 9. Observed advertised rents (bars) with a lognormal fitted by maximum likelihood to the interior region between $1,000 and $8,000 (line), extrapolated past both boundaries. The shaded area below $500 is the estimated missing mass. The upper bound is far into the tail and removes essentially nothing.] Figure 9: Figure 9. Observed advertised rents (bars) with a lognormal fitted by maximum likelihood to the interior region between $1,000 and $8,000 (line), extrapolated past both boundaries. The shaded area below $500 is the estimated missing mass. The upper bound is far into the tail and removes essentially nothing.
...
[figure: Figure 11. Each point is a complete re-estimation of the national index on a narrower band. The mechanism is confirmed: growth falls monotonically as the band excludes more of the market, from both directions, and the slope is estimated precisely. The extrapolation to an unfiltered sample is the open point at the left.] Figure 11: Figure 11. Each point is a complete re-estimation of the national index on a narrower band. The mechanism is confirmed: growth falls monotonically as the band excludes more of the market, from both directions, and the slope is estimated precisely. The extrapolation to an unfiltered sample is the open point at the left.
...
[figure: Figure 12. Decomposition of the ten-and-a-half point discrepancy against ZORI. Both mechanical explanations are real and both are negligible; the residual is not a rounding difference but the overwhelming majority of the gap.] Figure 12: Figure 12. Decomposition of the ten-and-a-half point discrepancy against ZORI. Both mechanical explanations are real and both are negligible; the residual is not a rounding difference but the overwhelming majority of the gap.
...
[figure: Figure 13. The same estimator, the same units and the same band, run once on the price each listing opened at and once on the price it carried when it left the market. The two paths are almost indistinguishable, and the five-year totals differ by roughly a fifth of a percentage point.] Figure 13: Figure 13. The same estimator, the same units and the same band, run once on the price each listing opened at and once on the price it carried when it left the market. The two paths are almost indistinguishable, and the five-year totals differ by roughly a fifth of a percentage point.
...
[figure: Figure 14. Each point is a metropolitan area. Landlords reset the asking price slightly upward at turnover and then give it back, several times over, while the unit sits. The two channels are positively correlated across metros, so they are not substitutes: soft markets do more of both.] Figure 14: Figure 14. Each point is a metropolitan area. Landlords reset the asking price slightly upward at turnover and then give it back, several times over, while the unit sits. The two channels are positively correlated across metros, so they are not substitutes: soft markets do more of both.
...
[figure: Figure 15. Listings per county on a log scale, all counties with any listing at all. The white areas are not places with no rental housing; they are places this platform does not index. Any national aggregate built from this sample weights the shaded counties and ignores the rest.] Figure 15: Figure 15. Listings per county on a log scale, all counties with any listing at all. The white areas are not places with no rental housing; they are places this platform does not index. Any national aggregate built from this sample weights the shaded counties and ignores the rest.

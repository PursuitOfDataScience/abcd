...
[figure: Every job was sorted into one of five groups by how much of its request it used, and each bar is a hundred per cent of the jobs. The memory bar is dominated by the two left-hand groups: 80 % of jobs never used more than a quarter of the memory they reserved, and the sliver on the right is the 1 % that used more than ninety per cent of it. The CPU bar is very different, and its right-hand block is the encouraging part: 32 % of jobs kept more than ninety per cent of their cores busy, nearly all of them small single-core tasks that asked for one core and used it. The 23 % on the left of the CPU bar is the same error as memory on a smaller scale, four or eight cores requested for work that used one.] Figure 1: Every job was sorted into one of five groups by how much of its request it used, and each bar is a hundred per cent of the jobs. The memory bar is dominated by the two left-hand groups: 80 % of jobs never used more than a quarter of the memory they reserved, and the sliver on the right is the 1 % that used more than ninety per cent of it. The CPU bar is very different, and its right-hand block is the encouraging part: 32 % of jobs kept more than ninety per cent of their cores busy, nearly all of them small single-core tasks that asked for one core and used it. The 23 % on the left of the CPU bar is the same error as memory on a smaller scale, four or eight cores requested for work that used one.
